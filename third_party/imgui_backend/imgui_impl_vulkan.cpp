...
static void ImGui_ImplVulkan_CreatePipeline(VkDevice device, const VkAllocationCallbacks* allocator, VkPipelineCache pipelineCache, VkRenderPass renderPass, VkSampleCountFlagBits MSAASamples, VkPipeline* pipeline, uint32_t subpass)
{
    ...
    color_attachments.resize(bd->VulkanInitInfo.ColorAttachmentCount);
    for (int i = 0; i < color_attachments.Size; ++i)
    {
        VkPipelineColorBlendAttachmentState& attachment = color_attachments[i];
    ...
    blend_info.attachmentCount = (uint32_t)color_attachments.Size;
...
bool    ImGui_ImplVulkan_Init(ImGui_ImplVulkan_InitInfo* info)
{
    ...
    if (bd->VulkanInitInfo.ColorAttachmentCount == 0)
        bd->VulkanInitInfo.ColorAttachmentCount = 1;
